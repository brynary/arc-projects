...
Expand the goal into a comprehensive game design spec for Fabro Racer Mini, a 3D voxel kart racer built with Three.js.
...
## Tracks (2 total, all unlocked)
Define 2 distinct tracks with unique themes, layouts, and hazards. Each needs:
...
## Items (3 total, one-item capacity)
Define 3 items with:
...
- 3 CPU opponents per race

## Race Structure
- Single Race mode only (no career/progression)
- Always 3 laps
- 1 human vs 3 CPU
- Both tracks available immediately

## Pre-Race Flow
- Track selection (2 tracks with previews)
- Character selection (4 characters with stat display)
