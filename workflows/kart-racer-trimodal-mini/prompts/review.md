Review the Fabro Racer Mini kart racer implementation against the spec.
...
### Content
- [ ] 2 distinct tracks (each with unique theme/layout/hazards)
- [ ] 4 characters with stat variations (Speed/Accel/Handling/Weight)
- [ ] 3 items with position-weighted distribution
- [ ] Mild item effects (max 1.2s loss of control, max 0.6s steering disabled)

### AI
- [ ] 3 CPU opponents racing
...
Opus writes to `.workflow/review_opus.md`. GPT writes to `.workflow/review_gpt.md`. Gemini writes to `.workflow/review_gemini.md`.
